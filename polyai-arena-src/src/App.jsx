...
async function callClaude(messages, model = "claude-sonnet-4-20250514") {
  ...
  const res = await fetch("/api/anthropic/v1/messages", {
    ...
    body: JSON.stringify({ model: claudeModel, max_tokens: 1000, messages }),
  });
...
function ProviderBadge({ provider, color }) {
  return (
    <span style={{ fontSize:"10px", fontWeight:600, letterSpacing:"0.05em", color, background:color+"15", padding:"2px 7px", borderRadius:"99px" }}>
      {provider}
    </span>
  );
}
...
function ModelPicker({ value, onChange, exclude }) {
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      <button onClick={() => setOpen(!open)} style={{ background:"#1A1A1A", border:"1px solid #2A2A2A", borderRadius:"10px", padding:"8px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:"8px", color:"#E8E8E8", fontSize:"13px", fontWeight:500, fontFamily:"inherit", minWidth:"180px" }}>
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, zIndex:100, background:"#1A1A1A", border:"1px solid #2A2A2A", borderRadius:"12px", padding:"6px", minWidth:"220px", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
...
function ChatBubble({ msg, modelColor }) {
  const isUser = msg.role === "user";
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:isUser?"flex-end":"flex-start", marginBottom:"16px" }}>
      {!isUser && <div style={{ fontSize:"11px", color:"#555", marginBottom:"5px", paddingLeft:"4px" }}><span style={{ color:modelColor, fontWeight:600 }}>●</span> AI</div>}
      <div style={{ maxWidth:"85%", background:isUser?"#1E1E1E":"transparent", border:isUser?"1px solid #2A2A2A":"none", borderRadius:isUser?"16px 16px 4px 16px":"0", padding:isUser?"11px 16px":"0 4px", color:"#E0E0E0", fontSize:"14px", lineHeight:"1.65", whiteSpace:"pre-wrap", wordBreak:"break-word" }}>
        {msg.content}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const endA = useRef(), endB = useRef(), textareaRef = useRef();

  useEffect(() => { endA.current?.scrollIntoView({ behavior:"smooth" }); }, [messagesA]);
  useEffect(() => { endB.current?.scrollIntoView({ behavior:"smooth" }); }, [messagesB]);

  const mA = MODELS.find(m => m.id === modelA) || MODELS[0];
  const mB = MODELS.find(m => m.id === modelB) || MODELS[1];

  const clearChat = () => { setMessagesA([]); setMessagesB([]); setVotedFor(null); setShowVote(false); };

  const sendMessage = async () => {
    if (!input.trim() || loadingA || loadingB) return;
    const text = input.trim();
    setInput(""); setVotedFor(null); setShowVote(false);

    if (tab === "chat") {
      const newMessages = [...messagesA, { role:"user", content:text }];
      setMessagesA(newMessages); setLoadingA(true);
      try {
        const reply = await callClaude(newMessages.map(m => ({ role:m.role, content:m.content })), modelA);
        setMessagesA([...newMessages, { role:"assistant", content:reply }]);
      } catch(e) {
        setMessagesA([...newMessages, { role:"assistant", content:"⚠️ Error: " + e.message }]);
      }
      setLoadingA(false);
    } else {
      const userMsg = { role:"user", content:text };
      const newA = [...messagesA, userMsg];
      const newB = [...messagesB, userMsg];
      setMessagesA(newA); setMessagesB(newB); setLoadingA(true); setLoadingB(true);
      const apiMsgs = newA.map(m => ({ role:m.role, content:m.content }));
      Promise.all([callClaude(apiMsgs, modelA), callClaude(apiMsgs, modelB)])
        .then(([rA, rB]) => {
          setMessagesA(a => [...a, { role:"assistant", content:rA }]);
          setMessagesB(b => [...b, { role:"assistant", content:rB }]);
          setLoadingA(false); setLoadingB(false); setShowVote(true);
        })
        .catch(() => { setLoadingA(false); setLoadingB(false); });
    }
  };
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"24px" }}>
          ...
          <div style={{ display:"flex", gap:"2px" }}>
            {[{id:"chat",label:"Chat"},{id:"battle",label:"⚔️ Battle"},{id:"leaderboard",label:"Leaderboard"}].map(t => (
              <button key={t.id} onClick={() => { setTab(t.id); clearChat(); }} style={{ background:tab===t.id?"#1E1E1E":"transparent", border:"none", borderRadius:"8px", color:tab===t.id?"#E0E0E0":"#666", padding:"6px 14px", cursor:"pointer", fontSize:"13px", fontWeight:500, fontFamily:"inherit" }}>{t.label}</button>
            ))}
          </div>
        </div>
        <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
          <button onClick={clearChat} style={{ background:"transparent", border:"1px solid #2A2A2A", borderRadius:"8px", color:"#666", padding:"5px 12px", cursor:"pointer", fontSize:"12px", fontFamily:"inherit" }}>New chat</button>
          ...
          <div style={{ maxWidth:"800px", margin:"0 auto" }}>
            ...
            <div style={{ background:"#131313", border:"1px solid #1E1E1E", borderRadius:"14px", overflow:"hidden" }}>
              ...
              {LEADERBOARD.map((row, i) => {
                ...
                const color = m?.color || "#7C7CFF";
                return (
                  <div key={i} style={{ display:"grid", gridTemplateColumns:"40px 1fr 90px 80px 80px 80px 70px", padding:"14px 20px", borderBottom:i<LEADERBOARD.length-1?"1px solid #181818":"none", alignItems:"center" }}
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                      ...
                      <div>
                        <div style={{ fontSize:"13px", fontWeight:500 }}>{row.model}</div>
                        <div style={{ fontSize:"11px", color:"#555", marginTop:"1px" }}>{row.provider}</div>
                      </div>
                    </div>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          {tab === "battle" && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderBottom:"1px solid #1A1A1A", flexShrink:0 }}>
              {[{model:modelA,set:setModelA,excl:modelB,br:true},{model:modelB,set:setModelB,excl:modelA,br:false}].map((s,i) => (
                <div key={i} style={{ padding:"12px 20px", display:"flex", alignItems:"center", gap:"12px", borderRight:s.br?"1px solid #1A1A1A":"none" }}>
                  <ModelPicker value={s.model} onChange={s.set} exclude={s.excl} />
                  <ProviderBadge provider={MODELS.find(m=>m.id===s.model)?.provider} color={MODELS.find(m=>m.id===s.model)?.color} />
                </div>
              ))}
            </div>
          )}
          {tab === "chat" && (
            <div style={{ padding:"12px 20px", borderBottom:"1px solid #1A1A1A", display:"flex", alignItems:"center", gap:"12px", flexShrink:0 }}>
              ...
              <ProviderBadge provider={mA.provider} color={mA.color} />
            </div>
          )}
          ...
          <div style={{ flex:1, overflow:"hidden", display:"flex" }}>
                      ...
                      <div style={{ display:"flex", flexWrap:"wrap", gap:"8px", justifyContent:"center" }}>
                        {SUGGESTIONS.map((s,i) => (
                          <button key={i} onClick={() => { setInput(s); textareaRef.current?.focus(); }}
                            style={{ background:"#151515", border:"1px solid #222", borderRadius:"10px", color:"#999", padding:"8px 14px", cursor:"pointer", fontSize:"12px", fontFamily:"inherit" }}
                            onMouseEnter={e=>{e.target.style.borderColor="#333";e.target.style.color="#E0E0E0";}}
                            onMouseLeave={e=>{e.target.style.borderColor="#222";e.target.style.color="#999";}}
                          >{s}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  {messagesA.map((msg,i) => <ChatBubble key={i} msg={msg} modelColor={mA.color} />)}
                  {loadingA && (
                    <div style={{ color:"#555", fontSize:"13px", marginBottom:"16px", display:"flex", alignItems:"center", gap:"6px" }}>
            ...
            {tab === "battle" && (
              <div style={{ flex:1, display:"grid", gridTemplateColumns:"1fr 1fr", overflow:"hidden" }}>
                {[{messages:messagesA,loading:loadingA,model:modelA,end:endA},{messages:messagesB,loading:loadingB,model:modelB,end:endB}].map((pane,i) => {
                  const m = MODELS.find(m=>m.id===pane.model)||MODELS[i];
                  return (
                    <div key={i} style={{ borderRight:i===0?"1px solid #1A1A1A":"none", overflow:"hidden", display:"flex", flexDirection:"column" }}>
                      <div style={{ flex:1, overflowY:"auto", padding:"20px" }}>
                        ...
                        {pane.messages.map((msg,j) => <ChatBubble key={j} msg={msg} modelColor={m.color} />)}
                        {pane.loading && (
                          <div style={{ color:"#555", fontSize:"13px", display:"flex", gap:"6px", alignItems:"center" }}>
          ...
          {showVote && !votedFor && tab==="battle" && (
            <div style={{ padding:"14px 20px", borderTop:"1px solid #1A1A1A", background:"#111", display:"flex", alignItems:"center", justifyContent:"center", gap:"12px", flexShrink:0 }}>
              ...
              {[{label:`👈 ${mA.label}`,winner:"A",color:mA.color},{label:"🤝 Tie",winner:"tie",color:"#666"},{label:`${mB.label} 👉`,winner:"B",color:mB.color}].map(v => (
                <button key={v.winner} onClick={() => setVotedFor(v.winner)} style={{ background:"#1A1A1A", border:`1px solid ${v.color}40`, borderRadius:"8px", color:v.color, padding:"7px 16px", cursor:"pointer", fontSize:"12px", fontFamily:"inherit", fontWeight:500 }}>{v.label}</button>
              ))}
            </div>
          )}
          {votedFor && tab==="battle" && (
            <div style={{ padding:"12px 20px", borderTop:"1px solid #1A1A1A", background:"#111", textAlign:"center", fontSize:"13px", color:"#555", flexShrink:0 }}>
              ✓ Vote recorded — {votedFor==="A"?mA.label:votedFor==="B"?mB.label:"Tie"} won this round
            </div>
          )}
          ...
          <div style={{ padding:"12px 20px 16px", background:"#0E0E0E", borderTop:"1px solid #1A1A1A", flexShrink:0 }}>
            <div style={{ maxWidth:tab==="battle"?"100%":"680px", margin:"0 auto" }}>
              <div style={{ background:"#151515", border:"1px solid #242424", borderRadius:"14px", display:"flex", alignItems:"flex-end", gap:"8px", padding:"10px 12px" }}>
                ...
                />
                <button onClick={sendMessage} disabled={!input.trim()||loadingA} style={{ width:32, height:32, borderRadius:"8px", border:"none", background:input.trim()&&!loadingA?"#7C7CFF":"#1E1E1E", color:input.trim()&&!loadingA?"#fff":"#444", cursor:input.trim()&&!loadingA?"pointer":"not-allowed", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
